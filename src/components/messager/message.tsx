...
export default function Message() {
    ...
    const currentUser = useSelector((state: RootState)=> state.userCurrent.value);
    ...
    useEffect(()=>{
        dispatch({type :ActionSaga.GET_MESSAGER_ROOM_ID, payload: {idRoom: room}})
    },[])
            ...
            <div className="h-full overflow-hidden py-4">
                <div className="h-full overflow-y-auto">
                    <div className="grid grid-cols-12 gap-y-2">
                        {listAllMessage.map((i: any) => {
                            return (
                                i.userId !== currentUser?.id ?
                                <TextMessageRight key={i.id} text={i.content} />
                                :
                                <TextMessageLeft key={i.id} text={i.content} />
