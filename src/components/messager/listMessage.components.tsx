...
import { TextMessageLeft, TextMessageRight } from "./textMessage.component";
import InputMessage from "./inputMessage";
import { useSelector } from "react-redux";
import { RootState } from "../../redux/store";


interface ListMessageInterface{
    sendMessage: Function;
}
export function ListMessageComponent(listMessage: ListMessageInterface) {

    const listAllMessage = useSelector((state: RootState) => state.message.value);
            ...
            <div className="h-full overflow-hidden py-4">
                <div className="h-full overflow-y-auto">
                    <div className="grid grid-cols-12 gap-y-2">
                        {listAllMessage.map((i: any, index) => {
                            return(
                                <TextMessageLeft key={index} text={i.message} />
                            )
                        })}
                       <TextMessageRight text="trinh cuto"/>
                        
                       
                    </div>
                </div>
            </div>
            <InputMessage sendMessage={listMessage.sendMessage} />
        </div>
    )
}